# Image Assets for CEO Reality Show Website

Place your images in this `public/images/` directory. The website expects the following images:

## Required Images

1. **hero-ceo.jpg** (Recommended: 1200x600px)
   - Hero section image showing CEO/business theme
   - Used in: Hero section right side

2. **about-show.jpg** (Recommended: 800x500px)
   - Image showing contestants in boardroom or business setting
   - Used in: About section - "What is the CEO Reality Show" card

3. **how-it-works.jpg** (Recommended: 800x500px)
   - Image showing the process or steps of the show
   - Used in: About section - "How the Reality Show Works" card

4. **prize-1.jpg through prize-6.jpg** (Recommended: 400x300px each)
   - Images for each prize card:
     - prize-1.jpg - Grand Prize (Cash)
     - prize-2.jpg - Brand New Car
     - prize-3.jpg - Prime Real Estate
     - prize-4.jpg - CEO Position
     - prize-5.jpg - Business Mentorship
     - prize-6.jpg - Media Exposure

## Image Guidelines

- **Format**: JPG or PNG
- **Optimization**: Compress images for web (use tools like TinyPNG or ImageOptim)
- **Alt Text**: All images have descriptive alt text already set
- **Fallback**: If images are missing, they will gracefully hide without breaking the layout

## Optional Images

You can also add:
- Sponsor logos
- Dr. Stephen Akintayo photos
- Contestant photos
- Behind-the-scenes images

All images should be optimized for web to ensure fast loading times.

